...
class ReasonsFilter(SimpleListFilter):
    ...
    Django's built-in filtering in the admin interface isnt designed to parse that string.
    This filter allows the admin to filter reports based on the condition choice reasons.
    
    Presents the Report.ALLOWED_REASONS choices in the sidebar and filters the
    queryset based on whether the selected reason appears in the comma-separated
    `reasons` field (using a case-insensitive `icontains` lookup).
    """
    title = 'Reasons'
    ...
    def queryset(self, request, queryset):
        ...
        Returns:
            QuerySet: Filtered reports where `reasons` contains the selected value,
            or the original queryset if no filter is applied.
        """
