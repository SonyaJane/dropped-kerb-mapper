...
- CustomSignupForm:
    ...
    a unique username, ask if the user is a wheeled mobility device user or if
    they care for someone who does, and capture mobility device usage details.
...
class ReportForm(forms.ModelForm):
    ...
    class Meta:
        ...
        - widgets: Dict mapping each field to a custom widget with specific
          attributes (e.g., #id, accept image rule) for integration with the
          map interface and frontend scripts.
        """
        model = Report
        fields = ('latitude', 'longitude', 'condition',
                  'reasons', 'comments', 'photo')
        widgets = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            ...
            HTML("""
                <div class="row hidden" id="reasons-container">
                    <div class="col-12 col-sm-4" id="reasons-label">
                        <label for="id_reasons" class="col-form-label">\
                            Reasons</label>
                    </div>
            ...
            Div(
                HTML('<label for="id_comments"\
                    class="col-12 col-sm-4 col-form-label">Comments</label>'),
            ...
            HTML("""
                {% if report.photo %}
                    <div class="form-group">
                        <label class = 'col-12 col-md-4 fw-normal mb-2'>\
                            Current Photo</label>
                        <img src="{{ report.photo.url }}" alt="Current Photo"\
                            class='col-12 col-md-8 current-photo'>
                    </div>
            ...
            Div(
                HTML(
                    ...
                    '{% else %}'
                    '<label for="id_photo"\
                        class="col-12 col-sm-4 col-form-label">Photo</label>'
                    '{% endif %}'
                ),
            ...
            Submit('submit', 'Submit',
                   css_class='btn btn-green report-submit-btn'),
            # Cancel button
            HTML("""
                {% if is_map_reports %}
                    <a href="javascript:void(0);"\
                        class="btn btn-mango report-cancel-btn">Cancel</a>
                {% else %}
                    {% if report.id %}
        ...
        if photo and hasattr(photo, 'read'):
            ...
            try:
                ...
                if output.getbuffer().nbytes > max_size:
                    # If it does, reduce the quality iteratively
                    # starting from 85
                    quality = 85
                    while output.getbuffer().nbytes > max_size and\
                            quality > 10:
                ...
                # Create a new InMemoryUploadedFile with
    ...
    def handle_delete_photo(self, instance):
        """
        Remove the existing photo from Cloudinary and clear it on the
        Report instance.
        ...
        Cloudinary photo resource (with a `public_id`), this method will:
          ...
          2. Clear `instance.photo` so the Report no longer references an
            image.

        Any exceptions from the Cloudinary API are caught and logged to the
        console.
    ...
    def save(self, commit=True):
        """
        Save the ReportForm, managing photo deletion and assignment before
        persisting.

        - Calls super().save(commit=False) to obtain an unsaved Report
          instance.
        - Uses handle_delete_photo() to remove the existing Cloudinary
          image if requested.
        - If a new photo was uploaded (in changed_data), assigns it to the
          instance.
        - If commit=True, saves the instance to the database and handles any
          m2m relations.

        Args:
            commit (bool): Whether to immediately save the Report instance to
            the database.
...
class CustomSignupForm(SignupForm):
    """
    Extends allauth's SignupForm to collect and persist additional user
    profile data during registration:

      - first_name (required)
      - last_name (required)
      - Auto-generates a unique username based on first and last name.
      - is_carer (bool): does the user care for someone who uses a wheeled
        mobility device?
      - uses_mobility_device (bool): does the user use a wheeled mobility
        device?
    ...
    is_carer = forms.TypedChoiceField(
            label="Do you care for someone who uses a\
                    wheeled mobility device?",
            required=False,
            choices=((False, 'No'), (True, 'Yes')),
            # convert the posted value to boolean
    ...
    mobility_device_type = forms.ChoiceField(
        label="Please select the primary mobility device that you,\
                or the person you care for, uses:",
    ...
    def __init__(self, *args, **kwargs):
        """
        Initialise the custom signup form with additional profile fields and
        Crispy styling.

        - Calls the parent SignupForm __init__() to set up default allauth
          fields.
        - Instantiates a Crispy Forms FormHelper for consistent Bootstrap
          layout.
    ...
    def save(self, request):
        ...
        - Normalises first_name and last_name, auto-generates a unique
          username.
        ...
        Args:
            request (HttpRequest): The request object for allauth's signup
            flow.
        Returns:
        ...
        user.uses_mobility_device =\
            self.cleaned_data.get('uses_mobility_device')
        user.mobility_device_type =\
            self.cleaned_data.get('mobility_device_type')
        user.save()
...
class ContactForm(forms.Form):
    ...
    Fields:
    - first_name (CharField): First name; auto-filled and read-only for
      authenticated users.
    - last_name  (CharField): Last name; auto-filled and read-only for
      authenticated users.
    - email      (EmailField): Email address; auto-filled and read-only for
      authenticated users.
    ...
    def __init__(self, *args, **kwargs):
        """
        Initialise the ContactForm, applying Bootstrap layout and optional
        user pre-fill.

        - Removes 'user' from kwargs to check for an authenticated user.
        - Calls the parent __init__() to set up fields.
        - Configures a Crispy Forms FormHelper for horizontal Bootstrap
          styling.
...
class CustomLoginForm(LoginForm):
    """
    Extends allauth's LoginForm to streamline the login UI and apply
    Crispy Forms styling:

      - Removes the “Remember Me” option by hiding its field and
